...
resource "aws_lambda_function" "function" {
  ...
  source_code_hash = var.code_archive == null ? filebase64sha256("code.zip") : filebase64sha256(var.code_archive)
  tracing_config {
    mode = "Active"
  }

  reserved_concurrent_executions = -1
  environment { # nosemgrep: aws-lambda-environment-unencrypted # By default, the AWS Lambda Environment is encrypted using AWS-managed keys, which is sufficient
