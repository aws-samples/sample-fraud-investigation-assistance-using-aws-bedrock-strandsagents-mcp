...
resource "aws_opensearchserverless_access_policy" "kb_access" {
  name = "${var.kb_oss_collection_name}-data"
  type = "data"
  policy = jsonencode([
    {
      Rules = [
...
resource "aws_opensearchserverless_security_policy" "kb_encryption" {
  name = "${var.kb_oss_collection_name}-enc"
  type = "encryption"
  policy = jsonencode({ # nosemgrep: aws-opensearchserverless-encrypted-with-cmk # AWS key encryption is sufficient
    Rules = [
      {
        Resource = [
          "collection/${var.kb_oss_collection_name}"
...
resource "aws_opensearchserverless_security_policy" "kb_network" {
  name = "${var.kb_oss_collection_name}-net"
  type = "network"
  policy = jsonencode([
    {
      Rules = [
